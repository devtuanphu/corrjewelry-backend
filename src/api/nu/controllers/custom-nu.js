module.exports = {
  async findAllProducts(ctx) {
    try {
      ...
      // Lấy nu từ Strapi (single type)
      const nu = await strapi.entityService.findOne("api::nu.nu", 1, {
        populate: {
          danh_muc_con: {
            populate: {
              danh_muc_chi_tiet: {
      ...
      if (!nu) {
        return ctx.notFound("nu không tồn tại");
      }
      ...
      nu.danh_muc_con.forEach((danhMucCon) => {
  ...
  async findAllDanhMucCon(ctx) {
    try {
      ...
      if (!nu) {
        return ctx.notFound("Không tìm thấy dữ liệu danh mục nu.");
      }

      const danhMucConList = nu.danh_muc_con.map((item) => ({
        id: item.id,
        title: item.title,
      }));
  ...
  async findProductsByCategorySlug(ctx) {
    try {
      ...
      // Kiểm tra nếu 'nu' không tồn tại
      if (!nu) {
        return ctx.notFound("nu không tồn tại");
      }

      // Lấy danh mục con (category) có slug tương ứng
      const danhMucCon = nu.danh_muc_con.find(
