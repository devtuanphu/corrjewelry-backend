module.exports = {
  async findAllProducts(ctx) {
    try {
      ...
      // Lấy unisex từ Strapi (single type)
      const unisex = await strapi.entityService.findOne(
        "api::unisex.unisex",
        1,
        {
          populate: {
      ...
      if (!unisex) {
        return ctx.notFound("unisex không tồn tại");
      }
      ...
      unisex.danh_muc_con.forEach((danhMucCon) => {
  ...
  async findAllDanhMucCon(ctx) {
    try {
      ...
      if (!unisex) {
        return ctx.notFound("Không tìm thấy dữ liệu danh mục unisex.");
      }

      const danhMucConList = unisex.danh_muc_con.map((item) => ({
        id: item.id,
        title: item.title,
      }));
  ...
  async findProductsByCategorySlug(ctx) {
    try {
      ...
      // Tìm đối tượng 'unisex' từ Strapi
      const unisex = await strapi.entityService.findOne(
        "api::unisex.unisex",
        1,
        {
      ...
      // Kiểm tra nếu 'unisex' không tồn tại
      if (!unisex) {
        return ctx.notFound("unisex không tồn tại");
      }

      // Lấy danh mục con (category) có slug tương ứng
      const danhMucCon = unisex.danh_muc_con.find(
  ...
  async findProductsByCategoryDetailSlug(ctx) {
    try {
      ...
      // Lấy đối tượng 'unisex' từ Strapi
      const unisex = await strapi.entityService.findOne(
        "api::unisex.unisex",
        1,
        {
